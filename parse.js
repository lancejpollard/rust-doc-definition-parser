...
const paths = fs.readdirSync('docs/tokio').map(x => `docs/tokio/${x}`).filter(x => x.endsWith('.json'))
...
paths.forEach(path => {
  ...
  fs.writeFileSync(`tmp/tokio/${slug}/base.link`, text.join('\n'))
})
...
function log(code, json, note) {
  ...
  json.def.where.forEach(lead => {
    switch (lead.base.type) {
      case 'fqn':
        getFqn(lead.base.head, 'lead').forEach(line => {
          leadText.push(line)
        })
        break
      case 'type':
        const name = paramCase(lead.base.head.name)
        leadText.push(`lead ${name}`)
        break
      case 'array':
  ...
  json.def.args.forEach(arg => {
    ...
    if (!arg.type?.prefix?.isRef) {
      argsText.push(`  cite free`)
    }
    if (arg.type?.prefix?.isMut) {
      argsText.push(`  lock free`)
    }
    if (arg.type?.prefix?.isDeref) {
      argsText.push(`  dive take`)
    }
    if (arg.type?.prefix?.isDyn) {
      argsText.push(`  dash take`)
    }
  })
